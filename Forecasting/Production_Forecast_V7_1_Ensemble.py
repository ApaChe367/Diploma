...
def main():
    ...
    # =============================================================================
    # FINAL COMPREHENSIVE SUMMARY (ONLY ONE!)
    # =============================================================================
    ...
    if len(evaluation_results) > 0:
        ...
        best_individual = max(evaluation_results, key=lambda x: x['r2'])
        print(f"\n🥇 Best Individual: {best_individual['model_name']} (R² = {best_individual['r2']:.4f})")
    
    # Add tuned PINN to comparison
    if 'eval_results' in locals() and eval_results:
        print(f"  {'Tuned PINN':15}: R² = {eval_results['r2']:.4f}, MAE = {eval_results['mae']:.0f} Wh")
    
    # ENSEMBLE RESULTS
    print("\n📊 ENSEMBLE MODEL PERFORMANCE:")
    if ensemble_result:
        print(f"  {ensemble_result['ensemble_name']:25}: R² = {ensemble_result['r2']:.4f}")
    if stacked_result:
        print(f"  {stacked_result['ensemble_name']:25}: R² = {stacked_result['r2']:.4f}")
    
    # Show optimal ensemble if created
    if 'optimal_result' in locals():
        print(f"  {'Optimal Weighted Ensemble':25}: R² = {optimal_result['r2']:.4f}")
    ...
    # Collect all results
    if len(evaluation_results) > 0:
        all_results.extend([(r['model_name'], r['r2']) for r in evaluation_results])
    if ensemble_result:
        all_results.append((ensemble_result['ensemble_name'], ensemble_result['r2']))
    if stacked_result:
        all_results.append((stacked_result['ensemble_name'], stacked_result['r2']))
    if 'optimal_result' in locals():
        all_results.append(("Optimal Weighted Ensemble", optimal_result['r2']))
    if 'eval_results' in locals() and eval_results:
        all_results.append(("Tuned PINN", eval_results['r2']))
    
    # Sort by R²
    all_results.sort(key=lambda x: x[1], reverse=True)
    
    for i, (name, r2) in enumerate(all_results[:5]):  # Top 5
        print(f"  {i+1}. {name:30}: R² = {r2:.4f}")
    
    # RECOMMENDATIONS
    print(f"\n💡 FINAL RECOMMENDATIONS:")
    if len(all_results) > 0:
        best_name, best_r2 = all_results[0]
        print(f"✅ Best model: {best_name} (R² = {best_r2:.4f})")
        
        if "ensemble" in best_name.lower():
            print("✅ The ensemble approach provided the best performance!")
        elif "lstm" in best_name.lower():
            print("✅ The LSTM model provided the best performance!")
            print("💡 Consider trying ensemble methods for potential improvement")
    ...
    if 'diverse_models_dir' in locals():
        print(f"   Diverse models: {diverse_models_dir}")
    if 'pipeline_save_path' in locals() and pipeline_save_path:
        print(f"   Production pipeline: {pipeline_save_path}")
...
def train_diverse_models(df, output_dir):
    ...
    ensemble_preds, actual_values = ensemble_weighted_average(trained_models, weights, test_loader, device)
    ...
    create_model_comparison_visualization(evaluation_results, [ensemble_result, stacked_result], model_dir)
