...
def constrained_objective_function(angles, df_subset, dni_extra, number_of_panels, 
    ...
    Parameters:
    - angles: [tilt, azimuth]
    - scenario_config: Dictionary with optimization configuration
    """
    try:
        tilt_angle, azimuth_angle = angles
        ...
        # Calculate constraint metric
        constraint_type = scenario_config.get('constraint_type', 'mismatch')
        constraint_value = calculate_constraint_metrics(df_temp, constraint_type)
        ...
        if constraint_type == 'mismatch':
            ...
            min_ratio = scenario_config.get('min_ratio', 50)  # 50% minimum
            if constraint_value < min_ratio:
                penalty = (min_ratio - constraint_value) * scenario_config.get('penalty_weight', 1000)
        
        # Define objective based on scenario
        ...
        if objective_type == 'maximize_production':
            ...
            # Simple economics proxy - can be improved with full NPV calculation
            economics_value = total_production * 0.20 - constraint_value * 0.001  # $/kWh electricity price
            objective_value = -economics_value
        else:
...
def run_constrained_optimization(df_subset, dni_extra, number_of_panels, inverter_params, 
    ...
    total_production = df_optimal['E_ac'].sum() / 1000
    constraint_value = calculate_constraint_metrics(df_optimal, scenario_config.get('constraint_type', 'mismatch'))
    self_sufficiency = calculate_constraint_metrics(df_optimal, 'self_sufficiency')
    self_consumption = calculate_constraint_metrics(df_optimal, 'self_consumption')
    ...
    logging.info(f"Optimization completed - Tilt: {optimal_angles[0]:.2f}°, Azimuth: {optimal_angles[1]:.2f}°")
    logging.info(f"Production: {total_production:,.0f} kWh, Constraint: {constraint_value:.2f}")
...
def get_predefined_scenarios():
    """
    Define predefined optimization scenarios.
    ...
    scenarios = {
        'maximize_production': {
            ...
            'max_mismatch': 150000,  # Loose constraint - 150 MWh mismatch allowed
            'penalty_weight': 1,
            'max_iterations': 100,
            'population_size': 15,
        ...
        'maximize_self_consumption': {
            ...
            'description': 'Optimize for maximum self-consumption with production constraint',
            'objective': 'minimize_mismatch',
            'constraint_type': 'self_consumption',
            'min_ratio': 75,  # Minimum 75% self-consumption
            'penalty_weight': 1000,
            'max_iterations': 150,
            'population_size': 20,
        ...
        'maximize_self_sufficiency': {
            ...
            'objective': 'minimize_mismatch',
            'constraint_type': 'self_sufficiency',
            'min_ratio': 80,  # Minimum 80% self-sufficiency
            'penalty_weight': 1000,
            'max_iterations': 150,
            'population_size': 20,
        ...
        'best_economics': {
            ...
            'constraint_type': 'mismatch',
            'max_mismatch': 100000,  # Stricter mismatch constraint
            'penalty_weight': 5,
            'max_iterations': 200,
            'population_size': 25,
            'weighting_strategy': 'adaptive_improved'
        ...
        'balanced_approach': {
            ...
            'objective': 'minimize_mismatch',
            'constraint_type': 'self_sufficiency',
            'min_ratio': 60,  # Minimum 60% self-sufficiency
            'penalty_weight': 100,
            'max_iterations': 150,
            'population_size': 20,
            'weighting_strategy': 'adaptive_improved'
...
def main():
    try:
        ...
        logging.info(f"Inverter pdc0 updated to: {inverter_params['pdc0']} W")
         ...
         # -------------------- Step 5A: Add Command Line Arguments for Scenarios --------------------
        # Add this to your argument parser section (around line 4940):
        ...
        if args.optimization_mode in ['scenarios', 'both']:
            logging.info("Running scenario-based optimization...")
            ...
            optimization_results['scenarios'] = scenario_results
        ...
        # Calculate key metrics for the selected configuration
        balanced_production = df['E_ac'].sum() / 1000  # kWh
        df_temp_mismatch = df.copy()
        df_temp_mismatch['load_wh'] = df_temp_mismatch['Load (kW)'] * 1000
        df_temp_mismatch['hourly_mismatch'] = df_temp_mismatch['E_ac'] - df_temp_mismatch['load_wh']
        df_temp_mismatch['weighted_mismatch'] = df_temp_mismatch['weighting_factor'] * np.abs(df_temp_mismatch['hourly_mismatch'] / 1000)
        balanced_weighted_mismatch = df_temp_mismatch['weighted_mismatch'].sum()
        
        logging.info(f"Selected Configuration Performance:")
        logging.info(f"  Total Production: {balanced_production:,.2f} kWh")
        logging.info(f"  Weighted Mismatch: {balanced_weighted_mismatch:,.2f} kWh")
        ...
        if 'scenario_name' in final_optimal:
            ...
            summary['Scenario_Self_Sufficiency_pct'] = f"{final_optimal['self_sufficiency_pct']:.2f}"
            summary['Scenario_Self_Consumption_pct'] = f"{final_optimal['self_consumption_pct']:.2f}"
            summary['Scenario_Constraint_Satisfied'] = str(final_optimal['constraint_satisfied'])
       ...
       # Save summary
        ...
        if args.optimization_mode == 'both' and 'scenarios' in optimization_results:
                # Add comparison of all methods
                summary['Available_Optimization_Methods'] = len(optimization_results['scenarios']) + 1  # +1 for multi-objective
...
if __name__ == "__main__":
    print("Constrained optimization and scenario system ready for integration.")
    print("Add the functions above to your pv_simulation_V17_6_.py file.")
    print("\nTo integrate with main():")
    print("Integration code ready.")
    print("Key features added:")
    print("1. Constrained optimization with penalty functions")
    print("2. Predefined scenarios (maximize production, self-consumption, economics, etc.)")
    print("3. Automatic scenario comparison and selection")
    print("4. Command line arguments for scenario control")
    print("5. Comprehensive comparison visualizations")
    print("\nTo integrate: Replace the optimization section in main() with the code above.")
    print(add_scenario_analysis_to_main())
